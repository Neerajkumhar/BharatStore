...
export function AsymmetricGallerySection({ config }: AsymmetricGallerySectionProps) {
  ...
  const images = (config.images && config.images.length > 0)
    ? config.images
    : config.imageUrl
    ? [config.imageUrl, ...defaultImages.slice(1)]
    : defaultImages;
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 items-center">
            {images.slice(0, 3).map((imgUrl, idx) => (
              <div
                ...
                className={`${
                  idx === 1
                    ? 'aspect-square rounded-3xl overflow-hidden shadow-xl bg-slate-200 md:scale-105 border-4 border-white'
                    : 'aspect-3/4 rounded-3xl overflow-hidden shadow-lg bg-slate-200'
                }`}
              >
