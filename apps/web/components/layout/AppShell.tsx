...
import { Menu, X, PlusCircle, Package, LayoutDashboard, ShoppingCart } from 'lucide-react';
import { TopNav } from './TopNav';
import { MobileRailNav } from './MobileRailNav';
import { RailNav } from './RailNav';
import { cn } from '@/lib/utils';

const RAIL_STORAGE_KEY = 'bs-rail-collapsed';
...
export function AppShell({ children, breadcrumbs, title, subtitle, action }: AppShellProps) {
  ...
  return (
    <div className="min-h-screen bg-bg-canvas flex flex-col">
      ...
      <div className="flex-1 flex items-stretch">
        {/* Desktop rail */}
        {!isBuilderRoute && <RailNav collapsed={railCollapsed} onToggleCollapsed={toggleRail} />}
      ...
      {/* Mobile slide-over menu (replaces VerticalNav) */}
      {!isBuilderRoute && mobileMenuOpen && (
        <div className="fixed inset-0 z-50 bg-slate-900/50 backdrop-blur-xs flex md:hidden">
          <div className="w-72 bg-white h-full flex flex-col shadow-2xl">
            ...
            <div className="flex-1 overflow-y-auto">
              <MobileRailNav onNavigate={() => setMobileMenuOpen(false)} />
            </div>
          </div>
...
export function DashboardShell(props: AppShellProps) {
